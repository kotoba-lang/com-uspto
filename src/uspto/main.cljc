(ns uspto.main
  "Kotodama WASM entrypoint for the USPTO clean-room actor (L4) — Clojure port.
  ...
  facts (`uspto.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"uspto.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "uspto-compat" :tier tier :entities entities} 200])
